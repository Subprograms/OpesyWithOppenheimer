...
static Instruction makeLeafInstr(std::vector<std::string>& vars)
{
    ...
    if (code == 0) {
        if (rng() & 1) return Instruction(OpCode::PRINT, "\"\"", "", "", false, false);
    ...
    if (code == 4) {
        if (vars.empty()) return makeLeafInstr(vars);
        std::string v = vars[rng() % vars.size()];
        uint32_t    a = Commands::getRandomInt(0, 0xFFF) & ~1;
        std::ostringstream ss; ss << std::hex << a;
        return Instruction(OpCode::WRITE, ss.str(), v, "", true, false);
    }
    if (code == 5) {
        std::string v = randVar(); vars.push_back(v);
        uint32_t a = Commands::getRandomInt(0, 0xFFF) & ~1;
        std::ostringstream ss; ss << std::hex << a;
        return Instruction(OpCode::READ, v, ss.str(), "", true, false);
    }
...
{
    static std::mt19937 rng{ std::random_device{}() };
    std::uniform_int_distribution<int> pickOp(0, 6);
    int code = pickOp(rng);

    switch (code)
    {
        case 0: {
            if ((rng() & 3) == 0) return Instruction{OpCode::PRINT, "\"\"", "", "", false, false};
            if (vars.empty()) {
                std::string v = randVar(); vars.push_back(v);
                return Instruction{OpCode::DECLARE, v,
                                   std::to_string(Commands::getRandomInt(0, 65535)),
        ...
        case 3: {
            ...
            bool rhs2Var = rng() & 1, rhs3Var = rng() & 1;
            ...
            std::string arg3 = rhs3Var ? vars[rng() % vars.size()]
                                       : std::to_string(Commands::getRandomInt(0, 500));
            return Instruction{code == 2 ? OpCode::ADD : OpCode::SUBTRACT,
                               target, arg2, arg3, rhs2Var, rhs3Var};
        }
        case 4: {
            std::string v = vars.empty() ? randVar() : vars[rng() % vars.size()];
            if (vars.empty()) vars.push_back(v);
            uint32_t    a = Commands::getRandomInt(0, 0xFFF) & ~1;
            std::ostringstream ss; ss << std::hex << a;
            return Instruction{OpCode::WRITE, ss.str(), v, "", true, false};
        }
        ...
        default: {
            uint8_t reps = static_cast<uint8_t>(Commands::getRandomInt(1, 3));
            std::vector<Instruction> body{ makeLeafInstr(vars) };
...
void Commands::vmstatCommand()
{
    if (!scheduler) {
        std::cout << "Scheduler not initialised.\n";
        return;
    }
    std::cout << scheduler->vmstatString();
}
...
void Commands::screenCommand(const std::string& cmdLine)
{
    ...
    if (subCmd == "-s") {
        iss >> procName >> memSize;
        if (procName.empty() || memSize <= 0) {
            std::cout << "ERROR: screen -s requires <name> <memSize>\n";
            return;
        }
        sSubCommand(procName, memSize);
    }
    else if (subCmd == "-r") {
        iss >> procName;
        if (procName.empty()) {
            std::cout << "ERROR: screen -r requires <name>\n";
            return;
        }
        rSubCommand(procName);
    }
    else if (subCmd == "-ls") {
        lsSubCommand();
    }
    else {
        std::cout << "ERROR: Invalid subcommand. Use -r | -s | -ls\n";
    }
}
...
{
    clearScreen();
    std::cout << "Attempting to reattach to process: " << name << '\n';

    for (auto& p : scheduler->getRunningProcesses())
        if (p.processName == name) { enterProcessScreen(scheduler->getProcess(name)); return; }

    for (auto& p : scheduler->getWaitingProcesses())
        if (p.processName == name) { enterProcessScreen(scheduler->getProcess(name)); return; }

    for (auto& e : scheduler->getFinishedProcesses())
        if (e.first.processName == name)
        {
            displayProcessSmi(const_cast<ProcessInfo&>(e.first));
    ...
    std::cout << "ERROR: Process \"" << name << "\" does not exist.\nPress Enter to return to the main menu";
    std::string _;
    std::getline(std::cin, _);
    clearScreen();
    menuView();
...
void Commands::sSubCommand(const std::string& name, int memSize)
{
    try {
        // if there's already a process of that name, just reattach
...
void Commands::batchLoop()
{
    ...
    while (batchRunning.load()) {
        ...
        for (auto const& p : scheduler->getWaitingProcesses())
            if (p.processName == pname) { exists = true; break; }
        if (!exists) for (auto const& p : scheduler->getRunningProcesses())
            if (p.processName == pname) { exists = true; break; }
        if (!exists) for (auto const& e : scheduler->getFinishedProcesses())
            if (e.first.processName == pname) { exists = true; break; }
        if (exists) { ++nextProcessID; continue; }
